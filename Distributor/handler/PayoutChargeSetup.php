...
include('../../Db/config.php');
 ...
 {  
      ...
      VALUES ('ADMIN','1','1','1','$packid','$start_range','$end_range','$tds','$gst','$comm_type','CREDIT','$retailer_comm','0','$charges','$distributor_comm','0')";  
      if(mysqli_query($con, $query))  
            {  
           echo '<script type="text/javascript">alert("PayoutChargeSetup Created")  
           location.replace("PayoutChargeSetup.php?pack_id='.$packid.'")
</script>';

      }else{
           echo '<script type="text/javascript">alert("Failed to Create PayoutChargeSetup")  
             location.replace("PayoutChargeSetup.php")
</script>';
...
$com_pack_id= $_POST['pid'];
 ...
 if($res->num_rows >0){
    ...
    <td><a href='#deleteEmployeeModal' class='delete' data-id='{$row['ID']}' data-toggle='modal'><i class='fas fa-trash'></i></i></a></td>
    </tr>";
