...
if(isset($_POST['paysprint'])){
    
   if(empty($paysprint_merchant)){
      ...
      Accepted : Do txn
      Pending : Onboarding complete , Please wait 6hr for AEPS Activation on Bank activation."
      Rejected : Onboarding Rejected by bank,
      
             
      **/       
      //{"response_code":1,"status":true,"message":"Onboarding completed","is_approved":"Accepted"}        
      if($rslt['status'] && strtolower($rslt['is_approved']) == "accepted"){
          
          $partner_id = $paysprint['PARTNER_ID'];
          $query = "INSERT INTO `aeps_merchant`(`REF_NO`, `TXN_ID`, `STATUS`, `MOBILE`, `PARTNERID`, `MERCHANTCODE`, `IS_ICICI_KYC`, `TIMESTAMP`, `OWNER`, `OWNER_ID`)
            VALUES ('STATIC','STATIC','1','$mobile','$partner_id','$merchantcode','1','$time','$owner','$owner_id')";
            $run_query = mysqli_query($con , $query);
      }        
      
      $paysprint_merchant = $con->query("SELECT * FROM `aeps_merchant` WHERE MOBILE='$mb' and STATUS='1' ORDER BY ID DESC LIMIT 1")->fetch_assoc();
  }  
  ...
  $toBoard = [
    'toBoard'=>empty($paysprint_merchant),
    'paysprint_api_cred'=>$paysprint,
    'paysprint_merchant_cred'=>$paysprint_merchant,
  ];
  echo json_encode($toBoard);
}
