...
if(isset($_POST['createupi'])){
    $usid= $_POST['usid'];
     $vauser = $con->query("select * from virtual_account where USER_ID='$usid' ORDER BY ID DESC LIMIT 1");
    if($vauser->num_rows == 0){
    ...
    $vaid = $us['MOBILE'];
    $data = json_encode([
            "virtualVpaId"=> strtoupper(trim(str_replace(" " , "" ,$us['FIRST_NAME']))).mt_rand(999 , 9999), 
            "name"=> $us['FIRST_NAME'], 
            "phone"=> $us['MOBILE'], 
            "email"=> $us['EMAIL']
...
function createVirtualAccount($usid , $requestId){
    global $con;
    $us = $con->query("select * from user where ID='$usid'")->fetch_assoc();
    $name = substr(str_replace(' ', '', $us['FIRST_NAME']), 0, 10);
    $url = "https://cac-api.cashfree.com/cac/v1/createVA";
    $vaid = $us['MOBILE'];
    $data = json_encode([
            "vAccountId"=> $vaid, 
...
function createupi($usid , $requestId){
    ...
    $url = "https://cac-api.cashfree.com/cac/v1/createVA";
    $vaid = $us['MOBILE'];
    $vpaid = strtoupper(substr(str_replace(" " , "" ,$us['FIRST_NAME']), 0, 10)).substr($us['MOBILE'] , 0 , 5);
    $data = json_encode([
            "virtualVpaId"=> $vpaid, 
            "name"=> $us['FIRST_NAME'], 
            "phone"=> $us['MOBILE'], 
...
if(isset($_POST["pageid"]) && $_POST["pageid"] == 1){
    ...
    if(empty($vauser) || $vauser == NULL){
        $vauser = $con->query("select * from virtual_account where USER_ID='$vid'")->fetch_assoc();
    }
    $upiid = $vauser["UPI"];
...
$virtual_acc = $con->query("SELECT * FROM `virtual_account` WHERE USER_ID='$usid' AND QR_RESPONSE IS NOT NULL ORDER BY ID DESC LIMIT 1")->fetch_assoc();
...
if($qrimg == null || $qrimg==""){
    ...
    $data = ["qr_image"=>$qrimg, "vpa"=>$row];
    
    echo json_encode(["status"=>true, "response_code"=>1, "message"=>"Qr Code Availbe", "receivableData"=>$data]);
    exit;
}
