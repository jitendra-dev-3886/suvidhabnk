...
if(isset($_POST['checkVerifyStatus'])){
 ...
 $data = array(
  ...
  $curl = curl_init();
            curl_setopt_array($curl, [
              CURLOPT_URL => $paysprint['URL']."/api/v1/service/payout/Payout/accountstatus",
  ...
  //{"response_code":1,"status":true,"accountstatus":"3","message":"Account List Fetched"}
  $rslt = json_decode($response, true);
  if($rslt['accountstatus'] == "0"){
      echo json_encode(['status'=>false, 'response_code'=>0, 'message'=>'Beneficiary Deactivated.']);
      exit;
  }
  else if($rslt['accountstatus'] == "1"){
      echo json_encode(['status'=>true, 'response_code'=>1, 'message'=>'Account has been succesfully activated']);
      exit;
  }
  else if($rslt['accountstatus'] == "2"){
      echo json_encode(['status'=>false, 'response_code'=>2, 'message'=>'Document Upload is']);
      exit;
  }
  else if($rslt['accountstatus'] == "3"){
      echo json_encode(['status'=>false, 'response_code'=>3, 'message'=>'Document verifcation pending at admin end']);
      exit;
  }
  else{
     echo $response;
...
$charge_amount = calc_com($amount , $usid , $usertype_id);
 if($user['AEPS_BAL'] < $amount+$charge_amount){
        echo $response = json_encode(array("response_code"=>  403 , "message" => "Insufficient Balance","status"=>false));
        exit;
}
...
if($user_bal >= 0){
        ...
        if($con->query($insert_report) && $con->query($deduct_bal) ){
            ...
            $st_msg = "Success,".$rslt->message;
              ...
              $con->query("update payout_transaction set RESPONSE='".str_replace("'" , "\'" , $response)."'  , STATUS='$st_msg' where REFFRENCE_ID='$refrence' ");
              insert_allreport($usid  ,$refrence , "PAYOUT" , $user['AEPS_BAL']  , $user['AEPS_BAL'] , $amount , "Failed" , "PAYOUT Transaction");
