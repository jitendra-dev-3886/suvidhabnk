...
//retialer commission 
function give_fastag_com($ref_id , $user_id , $usertype){
        global $con;
    
        $time = date("Y-m-d g:i:s A");
        $user_type_rows = $con->query("select * from user_type ")->num_rows;
        $trans = $con->query("select * from fastag_transaction where REFFRENCE_ID='$ref_id'")->fetch_assoc();
        ...
        //Inser into commission report
         $con->query("INSERT INTO `commission_report`(`SERVICE`, `REFFRENCE`, `USER_ID`, `USER_TYPE`, `AMOUNT`, `COMMISSION`,`GST` ,`TDS`, `TIME`) 
                    VALUES ('FASTAG','$ref_id','$user_id','46','".$trans['AMOUNT']."','$givenCom','$gst' ,'$tds' ,'$time')");
         
         $con->query("INSERT INTO `commission_report`(`SERVICE`, `REFFRENCE`, `USER_ID`, `USER_TYPE`, `AMOUNT`, `COMMISSION`,`GST` ,`TDS`, `TIME`) 
                    VALUES ('FASTAG','$ref_id','$ds_id','47','".$trans['AMOUNT']."','$ds_givenCom','$ds_gst' ,'$ds_tds' ,'$time')");
        
         $con->query("INSERT INTO `commission_report`(`SERVICE`, `REFFRENCE`, `USER_ID`, `USER_TYPE`, `AMOUNT`, `COMMISSION`,`GST` ,`TDS`, `TIME`) 
                    VALUES ('FASTAG','$ref_id','$ms_id','48','".$trans['AMOUNT']."','$ms_givenCom','$ms_gst' ,'$ms_tds' ,'$time')");
        
        // Insert All Report
        $con->query("update user set MAIN_BAL='$update_bal'  where ID='$user_id' ");
        // insert_allreport($user_id  ,$ref_id , "FASTAG Charge" ,$us_main_bal , $crnt_bal , $charge_amount , "Debit" , "FASTAG Transaction Charge" , "MAIN");
        insert_allreport($user_id  ,$ref_id , "FASTAG Commission" ,$crnt_bal , $update_bal , $givenCom , $pack['AMOUNT_TYPE'] , "FASTAG Transaction Commission" , "MAIN");
        $con->query("update user set AEPS_BAL='$ds_update_bal'  where ID='$ds_id' ");
        insert_allreport($ds_id  ,$ref_id , "FASTAG Commission" ,$ds_aeps_bal , $ds_update_bal , $ds_givenCom , $pack['AMOUNT_TYPE'] , "FASTAG Transaction Commission" , "AEPS");
        $con->query("update user set MAIN_BAL='$ms_update_bal'  where ID='$ms_id' ");
        insert_allreport($ms_id  ,$ref_id , "FASTAG Commission" ,$ms_main_bal , $ms_update_bal , $ms_givenCom , $pack['AMOUNT_TYPE'] , "FASTAG Transaction Commission" , "MAIN");
    // return true;
}
