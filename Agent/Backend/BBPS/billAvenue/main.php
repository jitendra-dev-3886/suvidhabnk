...
if(isset($_POST['gettAllOp'])){
    //Passing the category value in this post;
    $service = $_POST['gettAllOp'];
    
    $billers = $con->query("SELECT * FROM `billersdata` where blr_category_name='$service' order by blr_name ASC");
    echo '<option value="">Select Operator</option>';
     while($bilDT = $billers->fetch_assoc()){
           echo ' <option value="'.$bilDT['blr_id'].'">'.$bilDT['blr_name'].'</option>';                                     
        }
