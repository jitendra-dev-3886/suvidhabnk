...
include("Backend/AEPS/Paysprint/aeps_function.php"); // aeps use
 ...
 if(isset($_GET['MyLatestReport'])){
        $rep =  $con->query("SELECT * FROM `aeps_transactions` where USER_ID='$usid' order by ID Desc LIMIT 1")->fetch_assoc();
        $repId = $rep['REFFRENCE_ID'];
        $ttype = $rep['TRANS_TYPE'];
        
        if($ttype == 'MS'){
            
        header("location: Recipt/AePsMinistatment?refrence_id=$repId");
        
        }else{
            
        header("location: Recipt/AePsRecipt?refrence_id=$repId");
    }
          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">AePS Report Data Table</h3>
              </div>
              <!-- /.card-header -->
              <div class="row">
