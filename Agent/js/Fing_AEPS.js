...
//aeps onboard 
     $("#aepsonboard_form").submit(function(e){
         e.preventDefault();
             $("#loading_ajax").show();
             $.ajax({
                 url:'Backend/AEPS/Fingpay/main',
                 type:'post',
                 ...
                 success:function(data, st){
                    ...
                    console.log(data);
                     let rslt = JSON.parse(data);
                      let rs_code = rslt.data.merchants[0].status; 
                      let msg = rslt.data.merchants[0].remarks; 
                    //   let msg = rslt.message; 
                       if(rs_code == "success" || rs_code == "Successfully Updated"){
...
$("#sendotpbtn").click(function(){
     ...
     let ekycotp = "ekycotp";
             ...
             $.ajax({
                 url:'Backend/AEPS/Fingpay/main',
                 type:'post',
                 data: {adhar:$("#aadhar").val()  , kycpan:$("#kycpan").val()  , ekycotp , long:$("#long").val() , lat:$("#lati").val() },
                  beforeSend:function(xhr){xhr.setRequestHeader('Token', localStorage.getItem('Token'));},
                 success:function(data, st){
                     $("#loading_ajax").hide();
                    console.log(data);
                    ...
                    // console.log(rs_code);
                       if(rs_code === true){
                           $("#pkeyid").val(rslt.data.primaryKeyId);
                           $("#fptid").val(rslt.data.encodeFPTxnId);
                        $("#aadhar").attr("readonly" , "readonly");
                        $("#kycpan").attr("readonly" , "readonly");
                        $("#sendotparea").hide();
                        $(".otparea").show();
                        $("#resendotparea").show();
                        popup('success' , 'OTP SENT' , msg);
                        // location.reload();
                      } 
                      else{
                        popup('error' , 'OOPS..!' , msg);
...
$("#resendotpbtn").click(function(){
     $("#loading_ajax").show();
     let reekycotp = "ekycotp";

             $.ajax({
                 url:'Backend/AEPS/Fingpay/main',
                 type:'post',
                 data: {pkeyid:$("#pkeyid").val() , fptid:$("#fptid").val() ,adhar:$("#aadhar").val()  , reekycotp , long:$("#long").val() , lat:$("#lati").val() },
                  beforeSend:function(xhr){xhr.setRequestHeader('Token', localStorage.getItem('Token'));},
                 success:function(data, st){
                     $("#loading_ajax").hide();
                    console.log(data);
...
$("#validateotp").click(function(){
     ...
     let validateOTP = "validateOTP";
             $.ajax({
                 url:'Backend/AEPS/Fingpay/main',
                 type:'post',
                 data: {pkeyid:$("#pkeyid").val() ,aadhar:$("#aadhar").val() , fptid:$("#fptid").val() , otp:$("#otp").val() , validateOTP , long:$("#long").val() , lat:$("#lati").val() },
                  beforeSend:function(xhr){xhr.setRequestHeader('Token', localStorage.getItem('Token'));},
                 success:function(data, st){
                     $("#loading_ajax").hide();
                    console.log(data);
                    ...
                    // console.log(rs_code);
                       if(rs_code === true){
                        ...
                        $(".fingerarea").show();
                        $("#fingerdevices").show();
                        popup('success' , 'OTP Verified' , msg);
                        // location.reload();
                      } 
                      else{
                        popup('error' , 'OOPS..! Invalid OTP' , msg);
                      }
                 },
                 error:function(err){
                     $("#loading_ajax").hide();
...
$("#ekyc_form").submit(async function(e){
    e.preventDefault();
     $("#loading_ajax").show();
    ...
    if($("#txtPidData").val() != ""){
     ...
     let submitForm = "submitForm";
             $.ajax({
                 url:'Backend/AEPS/Fingpay/CompleteEKYC',
                 type:'post',
                  data: new FormData(this),
                 processData:false,
                 contentType:false,
                    ...
                    console.log(data);
                     let rslt = JSON.parse(data);
                      ...
                      let msg = rslt.message; 
                       ...
                       if(rs_code == "10000"){
                        popup_reload('success' , 'You are verified. ' , msg);
                        // location.reload();
                      } 
                      else{
                        popup('error' , 'OOPS..! Invalid OTP' , msg);
...
//aeps transfer 
     $("#aeps_form").submit(async function(e){
         e.preventDefault();
            let device = await aepssatrttxn();
            if($("#txtPidData").val() != ""){
             $("#loading_ajax").show();
             $.ajax({
                 url:'Backend/AEPS/Fingpay/Txns',
                 type:'post',
                 data: new FormData(this),
                 processData:false,
                 contentType:false,
                     ...
                     let rslt = JSON.parse(data);
                      ...
                      if(rs_code == 10000){
                          if($("#transType :selected").val() == "MS"){
                              let msm = rslt.data.miniStatementStructureModel;
                            ...
                            // popup('success' , 'Congratulations' , "<pre> Transaction Successfull. Statement : \n" + msg + "</pre>");
                              ...
                              msg += "\n Balance : "+rslt.data.balanceAmount;
                              Swal.fire({
                              title: 'Congratulations',
                              html:  "<pre> Transaction Successfull. Statement : \n" + msg + "</pre>",
                              icon: 'success',
     ...
     function check_aeps_status(id){
        //   preventDefault();
          ...
          let ref_id = id;
             $("#loading_ajax").show();
             $.ajax({
                 url:'Backend/AEPS/Paysprint/aeps_init_req',
                 type:'post',
...
var MethodInfo="";
var MethodCapture="";
var primaryUrl = "https://127.0.0.1:";// For HTTP	
...
// 			content: 'Please connect the device first!!',
// 		});
alert('Device is not ready. Try after reconnet the device.');
		SuccessFlag==1
		return false;								
	}	
	if(SuccessFlag==0)
...
// 		});
  ...
  function aepssatrt()
	   ...
	   var XML='<?xml version="1.0"?> <PidOptions ver="1.0"> <Opts fCount="1" fType="2" iCount="0" pCount="0" format="0" pidVer="2.0" timeout="10000" wadh="E0jzJ/P8UopUHAieZn8CKqS4WPMi5ZSYXgfnlfkWjrc=" posh="UNKNOWN" env="P" /> '+DString+'<CustOpts><Param name="mantrakey" value="" /></CustOpts> </PidOptions>';
