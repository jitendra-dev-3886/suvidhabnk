...
$res = $con->query("SELECT * FROM `user` WHERE `ID`='$my_id'")->fetch_assoc();
$pic = $res['PARTNER_ID'];
$profilepic = $con->query("SELECT * FROM `user_profile` WHERE `USER_ID`='$my_id'")->fetch_assoc();
...
<head>
  ...
  <title><?php echo $row['NAME']?> | IdentityCard</title>
</head>
<body>
...
.card header {
  ...
  box-shadow: inset 0 1px 0 0 rgba(255, 120, 120, .8),
...
<div class='card'>
  <header>
    <h1>Identity Card</h1>
    <img src="../assets/images/<?php echo $row['I_LOGO']?>" style="float: right; margin-top: -8%;" atl="logo" width="100">
  </header>
  <article>
    <img alt='My Pic' id='thumb' src='../admin/assets/Documents/<?php echo $pic ?>/<?php echo $profilepic['PROFILE_IMG']?>'>
    <h2><?php echo $res['FIRST_NAME']." ".$res['LAST_NAME']?></h2>
    <ul class="area">
      <li><b>ID</b> <?php echo $res['ID']?></li><br>
      <li><b>KIOSK Name</b> <?php echo $res['FIRST_NAME']." ".$res['LAST_NAME']?></li><br>
      <li><b>KIOSK Address</b> <?php echo $res['ADDRESS']?></li><br>
      <li><b>Mobile Number</b> <?php echo $res['MOBILE']?></li><br>
    </ul>
  </article>
</div>
<div>
    <button style="margin-top: 3%; margin-left: 50%;" onclick="window.print();" class="noPrint">Print</button>
</div>
</body>
</html>
