...
if(isset($_POST["pid"]) && $_POST["pid"] == 1){
    ...
    $fetchuser = $con->query("SELECT * FROM employee WHERE ID = '$id'")->fetch_assoc();
...
if(isset($_POST["pid"]) && $_POST["pid"] == 2){
    ...
    $fetchotp = $con->query("SELECT * FROM employee WHERE ID = '$id'")->fetch_assoc();
...
if(isset($_POST["pid"]) && $_POST["pid"] == 3){
    ...
    $admincheck = $con->query("SELECT * FROM employee WHERE ID = '$id' AND PASSWORD = '$cpass'")->fetch_assoc();
     
    
  
  if(!empty($admincheck)){
    $err = ["response_code"=>1, "message"=>"Response Recieved", "status"=>true]; 
    $con->query("UPDATE employee SET PASSWORD = '$npass' WHERE ID = '$id' AND PASSWORD = '$cpass'");
  }
  else{
      $err = ["response_code"=>3, "message"=>"Bad Response", "status"=>false];   
  }
