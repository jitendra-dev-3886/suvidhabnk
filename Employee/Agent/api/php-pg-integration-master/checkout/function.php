...
//retialer commission 
function givecom($ref_id , $user_id , $usertype , $ser){
        global $con;
        
        
        $time = date("Y-m-d g:i:s A");
        $user_type_rows = $con->query("select * from user_type ")->num_rows;
        $trans = $con->query("select * from wallet_cashfree where ORDER_ID='$ref_id'")->fetch_assoc();
        ...
        $ds_aeps_bal = $ds_data['MAIN_BAL'];
        $ms_main_bal = $ms_data['MAIN_BAL'];
        $compackage = $con->query("select * from commission_package where SERVICES='$ser'")->fetch_assoc();
        //fetch commission package id of retailer
        $com_id = $compackage["ID"];
        ...
        //Inser into commission report
         $con->query("INSERT INTO `commission_report`(`SERVICE`, `REFFRENCE`, `USER_ID`, `USER_TYPE`, `AMOUNT`, `COMMISSION`,`GST` ,`TDS`, `TIME`) 
                    VALUES ('$ser','$ref_id','$user_id','46','".$trans['AMOUNT']."','$givenCom','$gst' ,'$tds' ,'$time')");
         
         $con->query("INSERT INTO `commission_report`(`SERVICE`, `REFFRENCE`, `USER_ID`, `USER_TYPE`, `AMOUNT`, `COMMISSION`,`GST` ,`TDS`, `TIME`) 
                    VALUES ('$ser','$ref_id','$ds_id','47','".$trans['AMOUNT']."','$ds_givenCom','$ds_gst' ,'$ds_tds' ,'$time')");
        
         $con->query("INSERT INTO `commission_report`(`SERVICE`, `REFFRENCE`, `USER_ID`, `USER_TYPE`, `AMOUNT`, `COMMISSION`,`GST` ,`TDS`, `TIME`) 
                    VALUES ('$ser','$ref_id','$ms_id','48','".$trans['AMOUNT']."','$ms_givenCom','$ms_gst' ,'$ms_tds' ,'$time')");
        
        // Insert All Report
        $con->query("update user set MAIN_BAL='$crnt_bal'  where ID='$user_id' ");
        insert_allreport($user_id  ,$ref_id , "$ser Charge" ,$us_main_bal , $crnt_bal , $charge_amount , "Debit" , "$ser Transaction Charge" , "MAIN");
        
        $con->query("update user set MAIN_BAL='$update_bal'  where ID='$user_id' ");
        insert_allreport($user_id  ,$ref_id , "$ser Commission" ,$crnt_bal , $update_bal , $givenCom , $pack['AMOUNT_TYPE'] , "$ser Transaction Commission" , "MAIN");
        
        $con->query("update user set MAIN_BAL='$ds_update_bal'  where ID='$ds_id' ");
        insert_allreport($ds_id  ,$ref_id , "$ser Commission" ,$ds_aeps_bal , $ds_update_bal , $ds_givenCom , $pack['AMOUNT_TYPE'] , "$ser Transaction Commission" , "AEPS");
        $con->query("update user set MAIN_BAL='$ms_update_bal'  where ID='$ms_id' ");
        insert_allreport($ms_id  ,$ref_id , "$ser Commission" ,$ms_main_bal , $ms_update_bal , $ms_givenCom , $pack['AMOUNT_TYPE'] , "$ser Transaction Commission" , "MAIN");
    // return true;
}
