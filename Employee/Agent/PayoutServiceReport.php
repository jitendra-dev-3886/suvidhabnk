...
include("Backend/AEPS/Paysprint/aeps_function.php"); // aeps use
 ...
 if(isset($_GET['MyLatestReport'])){
        $rep =  $con->query("SELECT * FROM `payout_transaction` where USER_ID='$usid' order by ID Desc LIMIT 1")->fetch_assoc();
        $repId = $rep['REFFRENCE_ID'];
        header("location: Recipt/PayoutRecipt?refrence_id=$repId");
    }
