...
if($usBal > $send_amount){
        ...
        if($send_amount > 5000){
            ...
            {   
                ...
                $insert_report = "INSERT INTO `dmt_transactions`(`USER_ID`, `USER_TYPE`, `MOBILE`, `BENE_ID`, `ACCOUNT`, `TIMESTAMP`, `AMOUNT`, `TRANS_TYPE`, `REFFRENCE_ID`, `COMM_REFID`,`FILTER_DATE`, `APINAME`)
                ...
                if($con->query($insert_report) && $con->query($deduct_bal) ){
                    ...
                    $resRefId = $response['data']['referenceId'];
                          ...
                          if($subCode == "200" || $subCode == "201" || $subCode == "202"){
                              insert_allreport($usid  ,$refrence , "DMT" , $usBal  , $user_bal , $snd , "Debit" , "DMT Transaction", "MAIN");
                              
                                $sndam = number_format($snd , 2);
                                $usbl = number_format($user_bal , 2);
                                $usermb = substr($user['MOBILE'] , 7 , 10);
                                $mbmsg = urlencode("INR $sndam has been Debited from your Paydeer.in A/C No *******$usermb  towards DMT Txn. $refrence. Main Wallet Avl BAL is INR $user_bal. Team PayDeer");
                              ...
                              $con->query("update user set MAIN_BAL='$usBal' where ID='$usid' and USER_TYPE='$ustypeid' ");
                          }
                    }
                    else{
          ...
          $insert_report = "INSERT INTO `dmt_transactions`(`USER_ID`, `USER_TYPE`, `MOBILE`, `BENE_ID`, `ACCOUNT`, `TIMESTAMP`, `AMOUNT`, `TRANS_TYPE`, `REFFRENCE_ID` , `COMM_REFID`, `FILTER_DATE`, `APINAME`)
            ...
            $resRefId = $response['data']['referenceId'];
                  $con->query("update dmt_transactions set RESPONSE='".str_replace("'" , "\'" , $result)."'  , STATUS='$status' , RES_REFID='$resRefId'  where REFFRENCE_ID='$refrence' ");
                  if($subCode == "200" || $subCode == "201" || $subCode == "202"){
                      insert_allreport($usid  ,$refrence , "DMT" , $user['MAIN_BAL']  , $user_bal , $send_amount , "Debit" , "DMT Transaction", "MAIN");
...
$curl = curl_init();
    ...
    if(strtolower($status) == "rejected"){
       ...
       insert_allreport($usID  ,$refrence , "DMT Refund" , $user['MAIN_BAL']  , $refundBal , $txn['AMOUNT'] , "Credit" , "DMT Transaction Refund", "MAIN");
        ...
        $usermb = substr($user['MOBILE'] , 7 , 10);
         $mbmsg = urlencode("INR $sndam has been Credited from your Paydeer.in A/C No *******$usermb  towards DMT Refund Txn. $refrence. Main Wallet Avl BAL is INR $usbl. Team PayDeer");
          $smsrs2 = json_decode(sendSMS($user['MOBILE'] , $mbmsg , 1307164568497787783) , true);
      
      
      
        //  insert_allreport($usID  ,$refrence , "DMT Failed" , $user['MAIN_BAL']  , $refundBal , $txn['AMOUNT'] , "Credit" , "DMT Transaction Failed", "MAIN");
    }
