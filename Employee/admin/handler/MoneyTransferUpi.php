...
VALUES ('ADMIN','1','1','1','$user_comm','$com_name','$pack_name','UPI','SLAB','Active') ";

// echo "$sql";
// die();
$result = $con->query($sql);
if($result){
    echo "MoneyTransferUPICommissionSetup Succssfully Created .";
     
}else{
    echo "MoneyTransferUPICommissionSetup Doesn't Add Successfully!";
}
//  exit();
}
}
...
$i = 1;
$output = "";
      $res = $con->query("SELECT * FROM `commission_package` WHERE USER_TYPE='46' AND SERVICES='UPI' ORDER BY ID DESC");
 if($res->num_rows >0){
    ...
    <td>{$row['DATE']}</td>
    <td><a href='MoneyTransferChargeSetup.php?pack_id={$row['ID']}' class='badge badge-info right'>Go For Setup</a></td>
...
$i = 1;
$output = "";
      $res = $con->query("SELECT * FROM `commission_package` WHERE USER_TYPE='company' AND SERVICES='UPI' ORDER BY ID DESC");
 if($res->num_rows >0){
    while($row = $res->fetch_assoc()){
