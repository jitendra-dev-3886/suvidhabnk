 ...
 {  
    ...
    $query = "INSERT INTO `slab_commission`(`MAIN_OWNER`, `MAIN_OWNER_ID`, `OWNER`, `OWNER_ID`, `COMM_PACK_ID`, `MIN_AMOUNT`, `MAX_AMOUNT`, `TDS`, `GST`, `TYPE`, `AMOUNT_TYPE`, `AMOUNT`, `STATUS`, `CHARGE`, `DS_COM`, `MS_COM`) 
      ...
      if(mysqli_query($con, $query))  
            {  
           echo '<script type="text/javascript">alert("XDMTChargeSetup Created")  
           location.replace("XDMTChargeSetup.php?pack_id='.$packid.'")
                 </script>';

      }else{
           echo '<script type="text/javascript">alert("Failed to Create XDMTChargeSetup")  
             location.replace("XDMTChargeSetup.php")
                 </script>';
