...
if($pack_name != ""){
    ...
    VALUES ('ADMIN','1','1','1','$user_comm','$com_name','$pack_name','VIRTUAL_ACCOUNT','SLAB','Active') ";
    
    // echo "$sql";
    // die();
    $result = $con->query($sql);
    if($result){
        echo "Virtual Account Commission Setup Succssfully Created .";
         
    }else{
        echo "Virtual Account Commission Setup Doesn't Add Successfully!";
    }
}else{
        echo "Enter name !";
}
...
$i = 1;
$output = "";
      $res = $con->query("SELECT * FROM `commission_package` WHERE USER_TYPE='46' AND SERVICES='VIRTUAL_ACCOUNT' ORDER BY ID DESC");
 if($res->num_rows >0){
    ...
    <td>{$row['DATE']}</td>
    <td><a href='VirtualAccSlabSetup.php?pack_id={$row['ID']}' class='badge badge-info right'>Go For Setup</a></td>
...
$i = 1;
$output = "";
      $res = $con->query("SELECT * FROM `commission_package` WHERE USER_TYPE='company' AND SERVICES='VIRTUAL_ACCOUNT' ORDER BY ID DESC");
 if($res->num_rows >0){
    while($row = $res->fetch_assoc()){
