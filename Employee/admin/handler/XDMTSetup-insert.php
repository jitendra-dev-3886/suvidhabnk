...
VALUES ('ADMIN','1','1','1','$user_comm','$com_name','$pack_name','XDMT','SLAB','Active') ";

$result = $con->query($sql);
if($result){
    echo "XDMTServicesDMTCommissionSetup Succssfully Created .";
     
}else{
    echo "XDMTServicesDMTCommissionSetup Doesn't Add Successfully!";
}
//  exit();
}
}
...
$i = 1;
$output = "";
      $res = $con->query("SELECT * FROM `commission_package` WHERE USER_TYPE='46' AND SERVICES='XDMT' ORDER BY ID DESC");
 if($res->num_rows >0){
    ...
    <td>{$row['DATE']}</td>
    <td><a href='XDMTChargeSetup.php?pack_id={$row['ID']}' class='badge badge-info right'>Go For Setup</a></td>
...
$i = 1;
$output = "";
      $res = $con->query("SELECT * FROM `commission_package` WHERE USER_TYPE='company' AND SERVICES='XDMT' ORDER BY ID DESC");
 if($res->num_rows >0){
    while($row = $res->fetch_assoc()){
