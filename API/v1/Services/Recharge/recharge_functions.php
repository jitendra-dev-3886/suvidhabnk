...
if(isset($_POST['hlr_check'])){
    ...
    curl_setopt_array($curl, [
      CURLOPT_URL =>  $paysprint['URL']."/api/v1/service/recharge/hlrapi/hlrcheck",
    ...
    $rescode = $rstl["response_code"];
    if($rescode == 1){
    $operator = $rstl["info"]["operator"];
    $opData = $con->query("SELECT * FROM `switchOperator` WHERE PRODUCTNAME='$operator' AND (SERVICETYPE = 'Dth' OR SERVICETYPE = 'Prepaid') ORDER BY ID DESC LIMIT 1")->fetch_assoc();
    $opname = $opData["PRODUCTNAME"];
    $opcode = $opData["LONGCODE"];
    echo json_encode(["status"=>true, "response_code"=>1,"message"=>"Success", "operator"=>$opname, "opcode" => $opcode]);
}
}
...
function recharge($api_name ,$mb , $op , $am , $backup_api , $op_name , $txn_id , $opId ,$serviceType){
    ...
    if($url_p != ""){
            ...
            if($optional_p != ""){$req_url.= "&$optional_p&Account=$mb";}
            // echo $req_url;
            // exit;
                      ...
                      if($rs_code == 1){
                             insert_allreport($usid  ,$refrence , "Recharge" , $user['MAIN_BAL']  , $user_bal , $am , "Debit" , "Recharge Transaction", "MAIN");
                                ...
                                $usermb = substr($user['MOBILE'] , 7 , 10);
                                 $mbmsg = urlencode("INR $sndam has been Debited from your Paydeer.in A/C No *******$usermb  towards Recharge Txn. $refrence. Main Wallet Avl BAL is INR $usbl. Team PayDeer");
...
curl_close($curl);
    ...
    //   exit;
      ...
      if($status == "Failed"){
        ...
        $usbl = number_format($update_bal , 2);
        $usermb = substr($user['MOBILE'] , 7 , 10);
         $mbmsg = urlencode("INR $sndam has been Credited from your Paydeer.in A/C No *******$usermb  towards Recharge Refund Txn. $refrence. Main Wallet Avl BAL is INR $usbl. Team PayDeer");
          $smsrs2 = json_decode(sendSMS($user['MOBILE'] , $mbmsg , 1307164568497787783) , true);
