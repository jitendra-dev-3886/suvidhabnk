...
if($AccType = "VA")
{
    $result = json_decode(fetchVPAuser($VirutalID , $Name , $Mobile , $Email) , true);
...
{
    $result = json_decode(fetchUPIuser($VirutalVPAID , $Name , $Mobile , $Email) , true);
     
    $vap = $result['data']['virtualVPA'];
    $upaid = $result['data']['virtualVpaId'];
    
     $vamsg = $result['message'];
    if($result['SubCode'] == 200){
         $rspns = json_encode(array("response_code"=>111 , "message"=> "Account Fetched" , "RequestId"=> $refId , "data" => ["UPI"=> $upaid , "VaccountId" => $vap]));
    }
    else{
        $rspns = json_encode(array("response_code"=>113 , "message"=> "Account Not Found" , "RequestId"=> $refId , "data" => ["message"=> $vamsg , "ErrCode" => $result['SubCode']]));
    }
