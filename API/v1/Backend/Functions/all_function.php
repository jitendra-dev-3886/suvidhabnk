...
function ApiHit($req ,  $token="" , $res , $refId){
    global $con;
    if($token != ""){
        $tknData = json_decode(decrypt_token($token) , true);
        $usid = $tknData['USERID'];
    }
    else{
        $usid = "Not Found";
    }
    $con->query("INSERT INTO `ApiRequest`(`BODY`, `TOKEN`, `USID`, `TIME`, `DATE`, `REFID`, `RESPONSE_GIVE` , `IP`) 
    VALUES ('$req','$token','$usid','".date("g:i:s A")."','".date("Y-m-d")."','$refId','$res' , '".$_SERVER['REMOTE_ADDR']."' )");
    return $res;
}
...
$org = $result->org;
    ...
    `COUNTRY`, `STATE`, `CITY`, `ZIP`, `LATTITUDE`, `LONGITUDE`, `API_IP`, `INTERNET_ISP`, `INTERNET_ORG`, `MESSAGE`, `DATE` , `WALLET`, `ALL_MAIN`, `ALL_AEPS` , `APITYPE`) 
    VALUES
    ...
    '$city','$zip','','','$api_ip_address','$isp','$org','$msg','$time_stamp' , '$wallet', '$overallmainbal' ,  '$overallaepsbal' , 'API')");
