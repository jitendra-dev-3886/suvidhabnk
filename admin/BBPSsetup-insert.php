...
foreach($ar as $word){
 ...
 if($res->num_rows >0){
    ...
    <td>{$row['SERVICES']}</td>
    <td>{$row['DATE']}</td>
    <td><a href='SetBBPSOnlineBBPSCommission.php?pack_id={$row['ID']}' class='badge badge-info right'>Go For Setup</a></td>
...
$i = 1;
$output = "";
      $res = $con->query("SELECT * FROM `commission_package` WHERE USER_TYPE='company' AND SERVICES='BBPS' ORDER BY ID DESC");
 if($res->num_rows >0){
    while($row = $res->fetch_assoc()){
    $output .= "<tr>
    <td>".$i++."</td>
