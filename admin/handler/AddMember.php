...
if($_POST['mem_type']==1){
    ...
    $memfname= $_POST['memfname'];
	   ...
	   if($aepspack=="Default Package"){
	       $aepspack=83;
	   }else{
	       $aepspack;
	   }
	   
	   if($dmtpack=="Default Package"){
	       $dmtpack=89;
	   }else{
	       $dmtpack;
	   }
	   ...
	   if($payoutpack=="Default Package"){
	       $payoutpack=88;
	   }else{
	       $payoutpack;
	   }
	   
	   if($adhaarpaypack=="Default Package"){
	       $adhaarpaypack=84;
	   }else{
	       $adhaarpaypack;
	   }
	   
	   if($rechargepack=="Default Package"){
	       $rechargepack=85;
	   }else{
	       $rechargepack;
	   }
...
$sql1 = $con->query("INSERT INTO `user_profile`(`PROFILE_IMG`, `MAIN_OWNER`, `MAIN_OWNER_ID`, `OWNER_ID`, `USER_ID`, `ALTERNATE_PHONE_NO`, `DOB`, `GENDER`, `COUNTRY`, `STATE`, `FACEBOOK_URL`, `TWITTER_URL`, `LINKEDIN_URL`, `INSTAGRAM_URL`, `DRIBBLE_BOX_URL`, `BLOCK`, `DISTRICT`, `LAND_MARK`, `ACCOUNT_TYPE`, `UPI_ID`, `AADHAR_CARD_NO`, `PAN_CARD_NO`, `BANK`, `B_NAME`, `AC_HOLDER_NAME`, `AC_NUM`, `IFSC_CODE`, `PASSBOOK`, `PASSPORT_PIC`, `OTHER_DOCS`) VALUES ('$profile_name','ADMIN','1','1','$id','$mobile','','','','$state','','','','','','','','','','','$adharno','$panno','$bankName','$bankName','$acholder_name','$acc_no','$ifsc','','','')");
...
if($sql==TRUE){
	   ...
	   $msg="Dear $memfname $memlname, Your Suvidha Bankio Login ID is $mobile, and Password is $mempass, Thanks-Suvidha BANKio Team";
        ...
        $to = $email;
        $msg = "Congratulation! Dear $memfname $memlname , You are registered on Suvidhabnk Your Mobile number is: $mobile Password $mempass";
