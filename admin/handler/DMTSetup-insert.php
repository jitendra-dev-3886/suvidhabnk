...
VALUES ('ADMIN','1','1','1','$user_comm','$com_name','$pack_name','DMT','SLAB','Active') ";
...
if($result){
    echo "DMTServicesDMTCommissionSetup Succssfully Created .";
     
}else{
    echo "DMTServicesDMTCommissionSetup Doesn't Add Successfully!";
}
//  exit();
}
}
...
$i = 1;
$output = "";
      $res = $con->query("SELECT * FROM `commission_package` WHERE USER_TYPE='46' AND SERVICES='DMT' ORDER BY ID DESC");
 if($res->num_rows >0){
    ...
    <td>{$row['DATE']}</td>
    <td><a href='DMTChargeSetup.php?pack_id={$row['ID']}' class='badge badge-info right'>Go For Setup</a></td>
    <td>
    <a href='#' class='fas fa-edit' name='edit' data-toggle='modal' data-target='#exampleModaledit' id='{$row['id']}'></a>
    <a href='#deleteEmployeeModal' class='delete' data-id='{$row['ID']}' data-toggle='modal'><i class='fas fa-trash'></i></i></a>
    </td>
    </tr>";
...
$i = 1;
$output = "";
      $res = $con->query("SELECT * FROM `commission_package` WHERE USER_TYPE='company' AND SERVICES='DMT' ORDER BY ID DESC");
 if($res->num_rows >0){
    while($row = $res->fetch_assoc()){
