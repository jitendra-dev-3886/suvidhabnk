    ...
    if($_POST['page']=="All_Report")
    {
        ...
        if($res->num_rows > 0){
            ...
            //   $statusss=$recharge_table_query['STATUS'];
               //   $rc_tran['status']=$recharge_table_query['STATUS'];
              $status=$recharge_table_query['STATUS'];
              $statusss=explode(",", $status);
              if($statusss[0] =="Success")
                  {
                        $rc_tran['status']= '<span class="badge badge-success">'.Success.'</span>';
                  }
                  else if($statusss[0] =="Failed")
                  {
                        $rc_tran['status']= '<span class="badge badge-danger">'.Failed.'</span>';
                  }
            ...
            //   echo $m_operator;
             ...
             }else if($transaction_type=='DMT Commission' || $transaction_type=='DMT Charge' || $transaction_type=='DMT' || $transaction_type=='DMT Account Verify' || $transaction_type=='DMT Return' || $transaction_type=='DMT Charge Return' || $transaction_type=='DMT Comission Refund'){
            ...
            //   echo $m_operator;
               ...
               if($transaction_type=='DMT' || $transaction_type=='DMT Return'){
                 $rc_tran['transaction_details'] = "<span><b> DMT-$mobilee</span></b><br><span>Ref. No: $refid</span>/<span>$m_operator</span><br><span> Rs. $amountt Recharge for $mobilee</span><br><span>Web:<b>($web_ip)</b></span><br><span><button type='button' class='btn btn-danger edit_dmt_data' data-toggle='modal' data-target='dmt_exampleModal' data-eid='$refid' data-rid='$transaction_type' data-amount='$trans_amount' data-id='$tble_id'>Status Update</button>";
               }
            //   else{
            //      $rc_tran['transaction_details'] = "<span><b> DMT-$mobilee</span></b><br><span>Ref. No: $refid</span>/<span>$m_operator</span><br><span> Rs. $amountt Recharge for $mobilee</span><br><span>Web:<b>($web_ip)</b></span><br>";
            //   }
                 $rc_tran['api_res'] = $api_response;
            ...
            //   echo $m_operator;
             ...
             }else if($transaction_type=='PAYOUT'){
                $recharge_table_query=$con->query("SELECT * FROM `payout_transaction` WHERE `REFFRENCE_ID`='$refid'")->fetch_assoc();
               $mobilee=$recharge_table_query['MOBILE'];
               $api_response=$recharge_table_query['RESPONSE'];
               $m_operator=$recharge_table_query['OPERATOR'];
            //   $rc_tran['status']=$recharge_table_query['STATUS'];
            ...
            //   echo $m_operator;
               ...
               if($transaction_type=='PAYOUT' || $transaction_type=='PAYOUT Return'){
                 $rc_tran['transaction_details'] = "<span><b> PAYOUT-$mobilee</span></b><br><span>Ref. No: $refid</span>/<span>$m_operator</span><br><span> Rs. $amountt Recharge for $mobilee</span><br><span>Web:<b>($web_ip)</b></span><br><span><button type='button' class='btn btn-danger edit_payout_data' data-toggle='modal' data-target='payout_exampleModal' data-eid='$refid' data-rid='$transaction_type' data-amount='$trans_amount' data-id='$tble_id'>Status Update</button>";
               }
            //   else{
            //      $rc_tran['transaction_details'] = "<span><b> DMT-$mobilee</span></b><br><span>Ref. No: $refid</span>/<span>$m_operator</span><br><span> Rs. $amountt Recharge for $mobilee</span><br><span>Web:<b>($web_ip)</b></span><br>";
            //   }
            ...
            //   $rc_tran['status']=$sql['STATUS'];
                  if($statusss =="Transaction Successfull")
                  {
                        $rc_tran['status']= '<span class="badge badge-success">'.Success.'</span>';
                  }
                  else if($statusss =="Pending")
                  {
                        $rc_tran['status']= '<span class="badge badge-warning">'.Pending.'</span>';
                  }else if($statusss =="Failed"){
                        $rc_tran['status']= '<span class="badge badge-danger">'.Failed.'</span>';
                  }       
            //
            //   echo $dmt_status;
