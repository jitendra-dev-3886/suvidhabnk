...
include("../../Agent/Backend/PAYOUT/paysprint/payout_function.php");
    ...
    // ini_set("display_errors",1);
      ...
      $sql = "SELECT * FROM `payout_transaction` WHERE ID = '$editId'";  
      $query = $con->query($sql);  
      if ($query->num_rows > 0) {  
      $output = "";  
      ...
      $output .= "  
                        <form id='load_form'>
                         <div class='form-group'>
                            ...
                            <input type='number' class='form-control' name='mb' id='edit_mb' placeholder='Enter Mobile Number' value='{$row['ACCOUNT']}' readonly>
                          </div>
                         <div class='form-group'>
                  ...
                  <div class='modal-footer'>
                    ...
                    <button type='button' class='btn btn-primary' name='editSubmit' id='editSubmit_payout'>Submit</button>
                  </div>
                      </form>
";            
        }  
    ...
    if(isset($_POST['pageid']) && $_POST['pageid'] == 1){
        ...
        $fetch_rc_trans = $con->query("SELECT * FROM `payout_transaction` WHERE `ID`='$usId'")->fetch_assoc();
          ...
          // Commission Return from Commission table
            ...
            while($retailer_commm_roww = mysqli_fetch_assoc($retailer_commmm)){
               ...
               $update_user_bal=$con->query("UPDATE user SET `MAIN_BAL`='$comm_minus_ammount' WHERE ID='$user_idd'");
                ...
                insert_allreport($user_idd  ,$refId , "Payout Comission Refund" , $user_main_bal  , $comm_minus_ammount , $user_commission , "Debit" , "Debited Successfully" , "MAIN");
            ...
            $retailer_commmmm=$con->query("SELECT * FROM report WHERE REFERENCE_ID='$refId' AND FUND_TYPE='Debit' AND REMARK='PAYOUT Transaction'")->fetch_assoc();
        ...
        //       $update_user_bal=$con->query("UPDATE user SET `MAIN_BAL`='$new_ammount' WHERE ID='$user_iddd'");
         ...
         $update_DMT_Transaction_report=$con->query("UPDATE report SET PREVIOUS_AMOUNT='$user_main_ball', TRANS_TYPE='DMT Return', AMOUNT='$trans_ammountt', AFTER_AMOUNT='$new_ammounntt',FUND_TYPE='CREDIT'  WHERE REFERENCE_ID='$refId' AND USER_ID='$user_iddd' AND FUND_TYPE='Debit' AND REMARK='PAYOUT Transaction'");
    ...
    //   Charge Return     
            ...
            $retailer_commmmmm=$con->query("SELECT * FROM report WHERE REFERENCE_ID='$refId' AND FUND_TYPE='Debit' AND REMARK='Payout Transaction Charge'")->fetch_assoc();
         ...
         $update_payout_Transaction_Charge_report=$con->query("UPDATE report SET PREVIOUS_AMOUNT='$new_ammount', TRANS_TYPE='Payout Charge Return', AMOUNT='$trans_ammounntt', AFTER_AMOUNT='$new_ammountt',FUND_TYPE='CREDIT'  WHERE REFERENCE_ID='$refId' AND USER_ID='$userr_iddd' AND FUND_TYPE='Debit' AND REMARK='Payout Transaction Charge'");
            

         if ($update_payout_Transaction_Charge_report){  
               echo 1;  
          }else{  
               echo 0;  
          }   
      ...
      if($staus == "Success"){
          $retailer_comm=$con->query("SELECT * FROM report WHERE REFERENCE_ID='$refId'")->fetch_assoc();
            ...
            $trans = $con->query("select * from payout_transaction where REFERENCE_ID='$ref_id'")->fetch_assoc();
       ...
       //fetch user and its owner distributer and master distributer
        ...
        $main_bal=$update_ball;
            ...
            give_payout_com($ref_id , $user_id , $usertype);
